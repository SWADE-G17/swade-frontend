...
export async function fetchUsers(): Promise<Usuario[]> {
  ...
  if (!res.ok)
    throw new Error(`No se pudo obtener la lista de usuarios (${res.status}).`);
...
export async function updateUser(
  ...
  if (!res.ok)
    throw new Error(`No se pudo actualizar el usuario (${res.status}).`);
  return (await res.json()) as Usuario;
}
...
export async function deleteUser(id: string): Promise<void> {
  ...
  if (res.status === 404) throw new Error("Usuario no encontrado.");
  if (res.status !== 204)
    throw new Error(`No se pudo eliminar el usuario (${res.status}).`);
}
